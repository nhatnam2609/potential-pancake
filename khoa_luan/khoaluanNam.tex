...
\usepackage[style=numeric,backend=biber,natbib=true,maxbibnames=99, defernumbers=true]{biblatex}
\renewcommand*{\finalnamedelim}{\addcomma\space} % Replaces 'and' with a comma
...
\addbibresource{refen.bib}
\DeclareFieldFormat[mastersthesis]{type}{#1}
%\addbibresource{refen.bib}
...
%\vskip 2cm
%{\bf\fontsize{16pt}{22}\selectfont Người hướng dẫn: TS.Lâm Thị Thanh Tâm}\\TS.LÂM THỊ THANH TÂM
%\end{flushleft}
\begin{table}[!htb]
	\centering
...
Tôi cũng muốn cảm ơn Khoa Toán - Thống Kê đã tạo ra những cơ hội học thuật và nghiên cứu tốt cho tôi. Sự hỗ trợ và khuyến khích của quý thầy cô đã giúp tôi hoàn thành khóa luận này. Cuối cùng, tôi xin gửi lời cảm ơn sâu sắc đến gia đình và bạn bè của tôi. Sự ủng hộ và khích lệ không ngừng nghỉ của họ đã giúp tôi vượt qua những thách thức và hoàn thành công việc của mình.
Trong quá trình viết khóa luận này mặc dù đã được chỉnh sửa nhiều lần nhưng không thể tránh khỏi việc thiếu sót. Tôi xin chân thành cảm ơn nếu nhận được sự góp ý từ các quý thầy cô, anh chị và bạn bè để có thể chỉnh sửa luận văn được tốt hơn.
\end{justify}
\vskip 0.5cm
\begin{flushright}
...
\textit{Kỳ vọng} (expectation) của một biến ngẫu nhiên $\mathbf{x}$ được định nghĩa bởi:
	\begin{align}
		\label{eqn:30_21}
		\text{E}[\mathbf{x}] &= \sum_ \mathbf{x} \mathbf{x} p(\mathbf{x})  \hspace{0.7cm} \text{nếu}~\mathbf{ x} ~ \text{là rời rạc,} \quad
		\\
		\label{eqn:30_22}
...
\begin{figure}[t]
	...
	\end{subfigure}
	\caption{
		Đây là một ví dụ về kỳ vọng và phương sai. (a) Trong không gian một chiều, chúng ta chỉ xem xét một loại dữ liệu. (b) Trong không gian hai chiều, khi hai chiều không có tương quan, ma trận hiệp phương sai sẽ là một ma trận đường chéo, với $\sigma_1, \sigma_2$ là phần tử trên đường chéo. Đây cũng là các trị riêng của ma trận hiệp phương sai, đại diện cho phương sai của từng loại dữ liệu. (c) Trong không gian hai chiều khi hai chiều của dữ liệu có tương quan, chúng ta có thể tính kỳ vọng và phương sai trong từng chiều của dữ liệu. Phương sai lớn hơn cho thấy dữ liệu trong chiều đó phân tán hơn. Trong trường hợp này, dữ liệu theo chiều thứ hai phân tán hơn so với theo chiều nhất. }
	\label{fig:27_2}
\end{figure}
...
\begin{itemize}
\item[(i)] Ma trận hiệp phương sai là một ma trận đối xứng, hơn nữa, nó là một ma trận.	
...
\end{equation}
Để ý rằng ta có thể chọn $\mathbf{v}$ là vector đơn vị, i.e. $\|\mathbf{v}\|=1$. Với cách chọn như vậy, ta nhận được
...
%\in \R^K$ với $K < D$. Một cách đơn giản để giảm chiều dữ liệu từ $D$ về $K < D$ là chỉ giữ lại $K$ phần tử {quan trọng nhất}. 
%Có hai câu hỏi lập tức
%được đặt ra. 
%\begin{itemize}
%
%\item Thứ nhất, làm thế nào để xác định {tầm quan trọng} của
%mỗi phần tử? 
%\item Thứ hai, nếu tầm quan trọng của các phần tử là như
...
Bây giờ ta có 966 điểm dữ liệu huấn liệu và 322 điểm dữ liệu kiểm thử. Liệu ta có thể phân biệt được các nét đặc trưng của mỗi khuôn mặt chỉ thông qua các vector eigenface chỉ với 150 chiều thay và 1850 như ban đầu.
...
%\bibliographystyle{plain} % or {unsrt} for non-alphabetical order
%\bibliographystyle{plainnat}
%\bibliography{reference} % mybib.bib is the name of your .bib file
%\printbibliography[title={Tiếng Việt}, keyword=vietnamese]
\addcontentsline{toc}{chapter}{Tài liệu tham khảo}
\printbibheading[title={Tài liệu tham khảo}]

% The Vietnamese bibliography
...
%\printbibheading
%\printbibliography[notkeyword=vietnamese,heading=subbibliography,title={Other Sources}]
%\printbibliography[keyword=vietnamese,heading=subbibliography,title={vietnamese Sources}]
%\printbibliography[title={Tiếng Anh}, keyword=english]
%\subsection{Tiếng Việt}
%\nocite{*} % Ghi chú: Lệnh này sẽ "tham khảo" tất cả các mục trong file .bib, ngay cả khi chúng không được trích dẫn trực tiếp trong văn bản.
%\bibliography{refvn}
%
