...
- Bước 1: Chuẩn hóa dữ liệu sao cho $E\{\mathbf{x_i}\}=0$. với $\mathbf{x_i}$ là vector cột của của ma trận $\mathbf{X}$
...
\subsection{Ví dụ}
Quay lại ví dụ ở chương trước ta tìm PCA của ma trận $\mathbf{X}=\left[\begin{array}{ll}1 & 0 \\ 1 & 0 \\ 0 & 1\end{array}\right]$.

Lời giải:

- Bước 1: Chuẩn hóa ma trận $\mathbf{X}$ bằng các trừ mỗi điểm dữ liệu đi vector trung bình của toàn bộ dữ liệu để được dữ
liệu chuẩn hoá:
\begin{equation}
	\hat{\mathbf{x}}_n = \mathbf{x}_n - \bar{\mathbf{x}}
\end{equation}
với $\bar{\mathbf{x}} = \left[\frac{2}{3},\frac{1}{3} \right] $.

Ta có
$$
\bar{\mathbf{X}}=\left[\begin{array}{ll}
	\frac{1}{3} & \frac{-1}{3} \\
	\frac{1}{3} & \frac{-1}{3} \\
	\frac{-2}{3} & \frac{2}{3} \\
\end{array}\right]
